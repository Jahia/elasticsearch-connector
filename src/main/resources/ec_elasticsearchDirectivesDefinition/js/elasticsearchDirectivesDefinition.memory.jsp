<%@page contentType="text/javascript" %>
    ...
    (function () {
        ...
        var ConnectionMemoryUsageController = function ($scope, dcConnectionStatusService, i18n) {
            ...
            function init() {
                cmuc.connectionStatus = JSON.parse(dcConnectionStatusService.getCurrentConnectionStatus());
                initChart();
            ...
            function updateChartEntries(connectionStatus) {
                var heapUsed = connectionStatus.status.statistics.nodes.jvm.mem.heap_used_in_bytes / (1024 * 1024);
                heapUsed = heapUsed.toFixed(2);
                var entry = angular.copy(CHART_ENTRY_TEMPLATE);
                entry.c[1].v = heapUsed;
                entry.c[1].f = heapUsed + ' MB';

                if (cmuc.memoryUsageChart.data.rows.length === 60) {
